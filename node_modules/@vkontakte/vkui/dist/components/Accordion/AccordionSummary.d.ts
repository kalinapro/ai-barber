import * as React from 'react';
import { type SimpleCellProps } from '../SimpleCell/SimpleCell';
export interface AccordionSummaryProps extends Omit<SimpleCellProps, 'chevron'> {
    /**
     * Иконка для раскрытия контента.
     */
    ExpandIcon?: React.ElementType | undefined;
    /**
     * Иконка для сворачивания контента.
     */
    CollapseIcon?: React.ElementType | undefined;
    /**
     * Позиция иконки.
     */
    iconPosition?: 'before' | 'after' | 'none' | undefined;
}
export declare const AccordionSummary: ({ after, before, ExpandIcon, CollapseIcon, iconPosition, onClick, children, ...restProps }: AccordionSummaryProps) => React.JSX.Element;
//# sourceMappingURL=AccordionSummary.d.ts.map